...
class LomForm extends JPanel {
    ...
    public String toEvaluateFrame(String key, int cat, int subCat) {       
        String res = null;
        Object c = vComponents.elementAt(cat);
        
        if (c instanceof FormContainer)
                res = ((FormContainer) c).toEvaluateFrame(key, cat, subCat);
        else if (c instanceof MultiFormContainer)
                res = ((MultiFormContainer) c).toEvaluateFrame(key, cat, subCat);          
