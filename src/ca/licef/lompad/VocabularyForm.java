...
class VocabularyForm extends FormContainer {
        ...
        if (!selectedProfile.equals("IEEE")) {
            try {
                if (Util.externalProfile != null){
                    if (Util.externalProfile == "NORMETIC")
                        fillVocabularies(Util.resBundleProfileVocabulary, "x" + title, listVocab);
                } 
                else if (Util.LQMProfile == "LQM")
                    fillVocabularies(Util.resBundleLQMVocabulary, "Q" + title, listVocab);
    ...
    String toEvaluateFrame(String key) {
        /*String res = "";
        LangstringComponent c = (LangstringComponent) vComponents.elementAt(0);
        ...
        for (Enumeration e = vComponents.elements(); e.hasMoreElements();) {
            VocabularyComponent c = (VocabularyComponent) e.nextElement();
            String res = c.toEvaluateFrame(key);
            //if (res != null) xml += i + ".- " + res + "\n"; Esto es para múltiples campos
            if (res != null)
                xml += res + "\n";
        }        
    ...
    private boolean isControlValueMediatorNeeded(Element e) {
        ...
        if ((source.equals("LOMv1.0")) || (source.equals("LQMv1.0")))
            return true;
